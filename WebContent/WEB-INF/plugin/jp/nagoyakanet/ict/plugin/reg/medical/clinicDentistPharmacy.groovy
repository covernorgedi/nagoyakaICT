...
import jp.nagoyakanet.ict.scm.DocPrimaryCareOffice
...
class ClinicDentistPharmacy extends AbstractRegistryPage {

	private static final Log logger = LogFactory.getLog(ClinicDentistPharmacy.class)

	CreateAction createAction
	ConsumeAction consumeAction
	CreativeWork creativeWork

	List<DocPrimaryCareOffice> clinicList
	List<DocPrimaryCareOffice> dentistList
	List<DocPrimaryCareOffice> pharmacyList
	...
	void clear(DocUser signInUser, DocOffice signInOffice, DocTeam signInTeam, DocUser tgtUser) {
		...
		clinicList = [ new DocPrimaryCareOffice() ]
		clinicList[0].office = new DocOffice()
		clinicList[0].primaryCare = "U"
		clinicList[0].medicalSpecialtyList = []
		dentistList = [ new DocPrimaryCareOffice() ]
		dentistList[0].office = new DocOffice()
		dentistList[0].primaryCare = "U"
		pharmacyList = [ new DocPrimaryCareOffice() ]
		pharmacyList[0].office = new DocOffice()
		pharmacyList[0].primaryCare = "U"
	}
	...
	static boolean insertSamples(Map<String, User> users, Map<String, Office> offices, Map<String, Team> teams,
		...
		ClinicDentistPharmacy clinicDentistPharmacy01 = new ClinicDentistPharmacy()
		clinicDentistPharmacy01.clear(docUsers.get("武田　拓夫"), docOffices.get("山中病院"),
				docTeams.get("新川　光彦さんのケアチーム"), docUsers.get("新川　光彦"))
		Date createdAt01 = sdfYMDHMS.parse("2017-01-01 10:52:40")

		DocMinion dscdt01 = new DocMinion("reg.medical.clinicDentistPharmacy", "mnn", clinicDentistPharmacy01,
